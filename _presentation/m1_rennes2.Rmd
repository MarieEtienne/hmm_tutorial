...
* its transition kernel $\mathbb{P}i(,)$
...
* $\nu(k) =\mathbb{P}(Z_0=k)$
* $\mathbb{P}i(i,j)=\mathbb{P}(X_k=j\vert X_{k-1}=i)$ the transition matrix
...
* Probabilité d'observer une séquence
  $$\mathbb{P}(Z_0= z_0, Z_1=z_1, \ldots, Z_n = z_n) = \nu(z_0)\mathbb{P}i(z_0, z_1)\ldots \mathbb{P}i(z_{n-1}, z_n).$$
...
$$\alpha_k(j) = \sum_{s\in\mathcal{S} }\mathbb{P}_{\theta}(Y_{0:k}, Z_{k-1}=s , Z_k = s_j ) = \sum_{s\in\mathcal{S} }\mathbb{P}_{\theta}(Y_k\vert Z_k=s_j)\mathbb{P}_{\theta}(Y_{0:k-1}, Z_k Z_{k-1}=s_i)$$
...
$$\mathbb{E} \left( \log \mathbb{P}_{{ \gamma, \mathbb{P}i, \nu}}(Y_{0:n}, Z_{0:n})\vert Y_{0:n} \right)$$
